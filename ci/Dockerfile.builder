...
# bitoreum_hash
RUN git clone https://github.com/bitoreum/bitoreum_hash
RUN cd bitoreum_hash && python3 setup.py install
...
RUN useradd -u ${USER_ID} -g bitoreum -s /bin/bash -m -d /bitoreum bitoreum
...
RUN mkdir /bitoreum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /bitoreum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /bitoreum-src
